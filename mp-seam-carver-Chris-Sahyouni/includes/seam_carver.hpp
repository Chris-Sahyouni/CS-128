...
public:
  ...
  int CompareThree(int first, int second, int third) const;

  /**
   * Add any helper methods you may need
   */

  // THE ENERGY ARRAY STILL NEEDS TO BE DESTROYED

  void DestroyEnergyArray(int**& energy_array) const;
  void DestroyOldImage(ImagePPM& image) const;
  int** GetEnergyArray() const;
  void ConvertToVerticalSeamArray(int**& energy_array) const;
  void ConvertToHorizontalSeamArray(int**& energy_array) const;

  // REQUIRES DESTRUCTION
  int* TraceVerticalSeam(int**& energy_array) const;
  int* TraceHorizontalSeam(int**& energy_array) const;
